...
1. HEWL_anom_cut_friedelize_careless.sh
   ...
   -  Copies the integrated (unmerged) mtz file produced by the
      ...
      working directory
   -  Calls the cut_unmerged_mtz_by_frames.py utility to create mtzs
      with only a subset of the overall images
   -  Calls the friedelize.py utility to split the Friedel mates into
      two mtz files (\*_plus.mtz and \*_minus.mtz)

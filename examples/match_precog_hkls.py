...
#expt_file = "monochromatic.expt"
#refl_file = "monochromatic.refl"
expt_file = "optimized.expt"
...
dials_df = rs.DataSet({
    ...
    'Indexed' : [1] * len(refls), #np.asarray(refls.get_flags(refls.flags.indexed), dtype=bool),
...
for i in trange(len(elist)):
    ...
    aligned_hkls = align_hkls(dials_hkl, precog_hkl, precog_df.spacegroup) # rs.utils.hkl_to_asu for mapping to same asu, need hkls + spacegroup
...
#    aligned_hkls = precog_hkl
    ...
    if len(correct) > 0:
        percent_misindexed[i] = 100.*sum(~correct[im_dia['Indexed']])/len(correct[im_dia['Indexed']])
        percent_correct[i] = 100.*sum(correct[im_dia['Indexed']])/len(correct[im_dia['Indexed']])
...
frames_misindexed = pd.DataFrame({'Frame':np.arange(len(percent_misindexed)), '% Misindexed':percent_misindexed})
plt.figure()
...
plt.figure()
plt.scatter(both_df.loc[both_df.correct, 'Resolution_dia'].to_numpy(),
            both_df.loc[both_df.correct, 'Intensity_dia'].to_numpy(),
            alpha=0.2, label=labels[0])
plt.scatter(both_df.loc[both_df.incorrect, 'Resolution_dia'].to_numpy(),
            both_df.loc[both_df.incorrect, 'Intensity_dia'].to_numpy(),
            alpha=0.2, label=labels[1])
plt.scatter(both_df.loc[~both_df.Indexed_dia, 'Resolution_dia'].to_numpy(),
            both_df.loc[~both_df.Indexed_dia, 'Intensity_dia'].to_numpy(),
            alpha=0.2, label=labels[2])
ax = plt.gca()
ax.set_yscale('log')
...
manhattan_hkl_flattened = [i for i in manhattan_hkl_flattened if i != 0] # Only misindexed spots
plt.figure()
...
for i in range(len(manhattan_distances)):
    misindexed = []
    misindexed = [j for j in manhattan_hkl[i] if j != 0] # Only misindexed spots per image

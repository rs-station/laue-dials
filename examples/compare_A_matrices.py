...
def align_mats(crystal1, crystal2): # Unfinished
    ...
    """
    ref_orientation = crystal_orientation.crystal_orientation(crystal1.get_A(), crystal_orientation.basis_type.reciprocal)
    other_orientation = crystal_orientation.crystal_orientation(crystal2.get_A(), crystal_orientation.basis_type.reciprocal)
    ...
    try:
        ...
        return np.reshape(np.asarray(crystal1.get_A()), (3,3)), np.reshape(np.asarray(crystal2.get_A()), (3,3))
...
#dials_filename_expt = "stills.expt"
#dials_filename_expt = "monochromatic.expt"
dials_filename_expt = "optimized.expt"
#dials_filename_expt = "poly_refined.expt"
#dials_filename_expt = "aligned.expt"
#dials_filename_expt = "poly_refined.expt"
precog_filenames = sorted(glob.glob("precognition_files/input/e080_*.mccd.inp")) # "e080_001.mccd.inp"
plot_diffs = True
...
#        precog_matrix = np.reshape(precog_crystal_model.get_A(), (3,3))
    ...
    angle_diffs[i], angle_axes[i] = rot_angle(dials_crystal_model[i], precog_crystal_model) 
    da[i],db[i],dc[i],dalpha[i],dbeta[i],dgamma[i] = diff_cell_params(dials_crystal_model[i], precog_crystal_model)
...
for i in np.arange(len(precog_filenames)-2):
    dials_angles[i], dials_axes[i] = rot_angle(dials_crystal_model[i], dials_crystal_model[i+1]) 
ax = plt.figure().gca()
...
plt.savefig("frame_A_matrix_diffs/dials_diff_angles_per_frame.png")

# Plot angular changes in DIALS A matrix between frames
dials_angles = np.zeros(len(precog_filenames)-1)
dials_axes = np.ndarray(shape=(len(precog_filenames)-1,3), dtype=float)
for i in np.arange(len(precog_filenames)-1):
    dials_angles[i], dials_axes[i] = rot_angle(dials_crystal_model[0], dials_crystal_model[i]) 
ax = plt.figure().gca()
plt.plot(np.arange(len(dials_angles)),dials_angles)
plt.grid(axis='y', alpha=1)
plt.xlabel("Frames")
...
plt.savefig("frame_A_matrix_diffs/dials_diff_angles_per_frame_from_first.png")
